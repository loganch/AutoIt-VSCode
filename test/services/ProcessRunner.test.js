...
describe('ProcessRunner', () => {
  ...
  it('routes an asynchronous spawn error through the same exit/cleanup path as a normal exit', async () => {
    ...
    await runner.run('C:\\AutoIt\\AutoIt3.exe', ['script.au3']);

    expect(typeof childHandlers.error).toBe('function');

    await childHandlers.error(new Error('spawn ENOENT'));

    expect(hotkeyManager.reset).toHaveBeenCalled();
