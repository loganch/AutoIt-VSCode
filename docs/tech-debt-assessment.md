...
- [x] **F27. Module names don't match concepts** — Priority 2 (P1 × S2), Monitored, accidental — *Resolved: renamed `command_constants.js` → `commandTimings.js` (it's purely timing constants, F2/F19 already moved the registry/path concerns out of `ai_config.js`) and `commands/commandUtils.js` → `commands/editorActions.js` (it's editor-buffer helpers, not command wiring). `commandsList.js` (id list, used for the package.json/registry sync guard) and `commandRegistry.js` (id→handler map, F6) keep their names — content matches name, and they're now the only two "command*" modules, each with a distinct, accurate role. `ai_config.js` was left as-is: post-F2 it's genuinely a config facade (Proxy + path verification + listener registry), so the name fits; renaming the `ai_` prefix across all `ai_*.js` providers is a much larger, purely-cosmetic, repo-wide rename with little payoff for a priority-2 finding — deferred, not done here. 623 tests pass.*
  ...
  - **Remedy:** Rename to match responsibility (`autoItPaths.js`, `commandTimings.js`, `commandRegistry.js`, `editorActions.js`); drop or define the `ai_` prefix. ✓ `commandTimings.js`/`editorActions.js` done. `commandRegistry.js` already existed (F6) with a different role than this remedy assumed, so `commandsList.js` was left alone rather than colliding with it. `ai_` prefix removal deferred as repo-wide cosmetic churn out of proportion to a Monitored finding.
...
**Recommended focus:** Change Propagation (4.5) and Dependency Disorder (4.0) remain the highest average priority groups, though both are fully resolved. All 28 findings are now resolved and verified with passing tests (F27's `ai_` prefix removal was scoped down to the genuinely misnamed modules; full prefix removal across all `ai_*.js` providers is deferred as low-value cosmetic churn).
