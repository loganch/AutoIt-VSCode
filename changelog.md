...
- **Stale Au3Check diagnostics**: Au3Check results are now cached and correctly invalidated when configuration changes, so the Problems panel no longer shows stale output (cad393e)
- **Completion and signature provider robustness**: hardened include path handling, include parsing, and include-data retrieval error handling (eac3bc4, 0e76789, 10dc0c8)
...
- **Completion and signature caching**: cache library include completions and document signatures per document for faster IntelliSense (76ff99f, 10dc0c8)
